...
defined( 'JETPACK__API_BASE' ) or define( 'JETPACK__API_BASE', 'emptybase' );
function is_jetpack_active () {
	if ( class_exists('Jetpack') && JETPACK__API_BASE == 'https://jetpack.wordpress.com/jetpack.' ) {
		return 1;
	} else {
	return 0;
	}
...
if ( is_jetpack_active() ) {
	wp_die( 'Jetpack is activated. Slimjetpack cannot be used together with Jetpack!' );
	exit;
}
...
/* Admin Pages */
	...
	function admin_menu() {
		...
		&&
			...
			$title = __( 'Slim Jetpack', 'slim jetpack' );
		}
		if ( current_user_can( 'manage_options' ) ) {
		$hook = add_submenu_page( 'options-general.php', 'Slim Jetpack', $title, 'read', 'jetpack', array( $this, 'admin_page' ), 'div' );
